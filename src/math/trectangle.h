...
static inline void TRectangleSet(TRectangle *r, TInt32 x, TInt32 y, TSize w, TSize h) {
	r->x = x;
	r->y = y;
	r->w = w;
	r->h = h;
...
static inline unsigned char TRectangleContainsPointF(const TRectangle *r, const TPoint *pt) {
...
static inline unsigned char TRectangleXContainsPoint(const TRectangle *r, TInt32 x, TInt32 y) {
	return (r->x + (int)r->w >= x && r->x <= x);
}

static inline unsigned char TRectangleYContainsPoint(const TRectangle *r, TInt32 x, TInt32 y) {
...
static inline void TRectangleMove(TRectangle *r, TInt32 x, TInt32 y) {
	r->x = x;
	r->y = y;
}
